...
public:
	...
	void setHostMem(size_t size,T *hptr, bool vbo, bool doubleBuffer, bool useElementArray);
private:
...
GpuArray<T>::copy(Direction dir, uint start, uint count)
{
	...
	case DEVICE_TO_DEVICE:
		cutilSafeCall(cudaMemcpy((void *) (m_dptr[m_currentWrite] + start), (void *) (m_dptr[m_currentRead] + start), count*sizeof(T), cudaMemcpyDeviceToDevice));
		break;
    }
    unmap();
}
...
template <class T> 
void
GpuArray<T>::setHostMem(size_t size,T *hptr, bool vbo, bool doubleBuffer, bool useElementArray)
{
	m_size = size;
